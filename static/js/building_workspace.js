const slug = document.getElementById('tenant-data').dataset.slug;
...
// Nombre total d'étapes dans le HTML (step-0 à step-6)
const TOTAL_STEPS = 7;

let lastStepIndex = -1;

/**
 * Met à jour l'affichage des étapes selon l'index courant.
 * - Étapes < stepIndex  → "done"
 * - Étape  = stepIndex  → "active" (spinner)
 * - Étapes > stepIndex  → "pending"
 * - isFinal = true      → toutes "done"
 */
function updateSteps(stepIndex, isFinal) {
  ...
  const completed = isFinal ? TOTAL_STEPS : stepIndex;
  const pct = Math.round((completed / TOTAL_STEPS) * 100);
  if (progressBar)  progressBar.style.width   = `${pct}%`;
  if (progressLabel) progressLabel.textContent = `${completed} / ${TOTAL_STEPS} steps completed`;
}

/** Transition douce sur le sous-texte */
...
function showSuccess(url) {
  ...
  title.textContent = 'Workspace prêt !';
  updateSteps(0, true);
...
async function pollStatus() {
  ...
  try {
    ...
    // status === 'provisioning' → mettre à jour les étapes visuelles en temps réel
    const stepIndex = typeof data.step_index === 'number' ? data.step_index : 0;
    if (stepIndex !== lastStepIndex) {
      lastStepIndex = stepIndex;
      updateSteps(stepIndex, false);
    }
    if (data.message) setSubtext(data.message);

  } catch (e) {
    // Erreur réseau temporaire → continuer silencieusement
    console.warn('[BuildingWorkspace] Poll error:', e);
  }
}

// Premier appel immédiat, puis toutes les 3s
pollStatus();
